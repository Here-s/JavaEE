public class Test3 {
    ...
    //    四次挥手就是各自和对方说断开连接。
    //  三次握手中：B 发生的 ACK 和 SYN 是同一时机发生的，就能够合并，此处的 B 给 A 发送的 ACK 和 SYN
    //   都是操作系统内核负责的，B 给 A 发的 FIN 是用户代码负责（B 的代码中调用了 socket.close()方法，才会触发

    //状态转换：
    // CLOSE_WAIT：四次挥手挥了两次之后出现的状态，这个状态就是在对待代码中调用 socket.close 方法，来进行后续的
    //  挥手过程，正常情况下，一个服务器说明不应该存在大量的 CLOSE.WAIT 如果存在，说明大概率是代码 bug，
    //   close 没有被执行到
    // TIME_WAIT：谁主动发起 FIN，谁就进入 TIME_WAIT 起到的效果：就是给最后一次 ACK 提供重传机会，表面上看起来
    //  A 发送完 ACK 之后，就没有 A 的啥事了。按理说 A 就应该销毁链接，释放资源了，但是并没有直接释放，而是会进入
    //  TIME_WAIT 状态等待一段时间，一段时间后，再来释放资源。等这一会儿，就是怕最后一个 ACK 丢包了，如果丢包了，
    //   就意味着 B 过一会儿就会重传 FIN。如果最后一个 ACK 丢了，B 就无法区别是 FIN 丢了，还是 ACK 丢了
    //   于是 B 就假设 FIN 丢了，就重传 FIN（超时重传）。因为如果断开的太早，就没办法继续重传了。
    //   持续时间：2*MSL  MSL：表示网络上任意两点之间，传输需要的最大时间。这个时间也是系统上可以配置的参数
    //   一个典型的设置就是：60s（经验值）

    //TCP 的可靠性是最高的，但是 TCP 也会尽可能的提高传输效率

    //滑动窗口：存在的意义是，保证可靠性的前提下，尽量提高传输效率。就是一次发一波数据，然后等一波 ACK
    // 如果一次批量发送的数据位 N，统一等待一波，那么 N 就是窗口大小。等到第一波的第一组 ACK 之后，就发送下一组数据
    // 滑动窗口越大，就可以认为传输速度越快，因为窗口大了，同一时间等待的 ACK 就更多，总的等待 ACK 就少了。
    //如果滑动窗口丢包问题，如果能接收到更后面的 ACK，那么前面的 ACK 就已经收到了。如果丢了 ACK 不用担心
    // 因为后面的内容会涵盖 ACK。
    //如果滑动窗口数据丢了，那么接收方就会反复索要 丢失的数据，即使发送方已经发了，接收方还是会继续索要，所以就触发了 重传
    // 在重传之前，会先把数据传传一部分，然后把缺的数据补齐，就继续往后传新的数据。只是把丢了的那一块数据重传即可，
    // 整体的重传效率还是比较高的，就叫做快重传。

    //流量控制：滑动窗口的延申，目的是为了可靠性。滑动窗口越大，传输速率越高，不光要考虑发送方，还得考虑接收方。
    // 发送方发的太快，接收方处理不过来，就会把新收的包给丢了，发送方还得重传。
    // 流量控制的关键：得能够衡量接收方的速度，直接使用接收方接受缓冲区的剩余空间大小，来衡量当前的处理能力。
    // 可以把接收缓冲区理解为一个阻塞队列，B 的应用程序，在调用 read 方法的时候，就是从接收缓冲区中来取数据。
    // 通过 ACK 报文来看缓冲区大小，如果接收方反馈的窗口大小是 0，那么发送方也会发一个信息，用来探测有没有空间。

    //拥塞控制：也是滑动窗口的延申，也是限制滑动窗口的发送速率。控制衡量的是：发送方到接收方 整个链路之间的拥堵情况，
    // 都会影响传输速率。开始的时候，以一个比较小的窗口来发送数据，如果数据很流畅的到达了，那么就加大窗口大小，
    // 加大到一定程度之后，出现了丢包（就意味着链路出现拥堵了），然后再减小窗口。通过反复的 增大/减小 最终达到”动态平衡“
    // 通过拥塞窗口来限制滑动窗口的大小。最终滑动窗口的大小 由 拥塞窗口 和 流量控制窗口共同决定，由小的决定
    // 一旦丢包，就立刻让窗口变小，降低速度，然后继续去试探的增大。如果把窗口一下变得很小，就是期望这次传输，一定能成功。
    // 阈值：线性和指数之间变化的

    //延时应答：相当于流量控制的延申，流量控制是踩了下刹车，使发送方发的不要太快，

    //捎带应答：客户端和服务器之间的通信：由以下几种模型：
    // 1、一问一答，客户端发一个请求，服务器返回一个对应的响应。
    // 2、多问一答：上传文件  最常用的就是这个模型
    // 3、一问多达：下载文件
    // 4、多问多答：直播，串流

    //面向字节流：粘包问题（针对面向字节流），指的是在应用层数据报，在 TCP 缓冲区中，若干个 应用层数据报混在一起了，
    // 分不出来谁是谁

    //TCP 的异常处理：
    // 1、进程终止：触发关闭文件操作，很正常的关闭
    // 2、机器关机：杀进程的放式
    // 3、机器掉电/网线断开：发送方掉电和接收方掉电 分别处理。

    //UDP 传输效率高，但是可靠性比 TCP 低。如何使用 UDP 实现可靠传输：在应用层的代码中抄 TCP 作业
}
