...
public class Test2 {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        //  加锁操作就是把多个操作打包成原子
        // 4、内存可见性，也会影响到线程安全。举例：针对同一个变量，一个线程进行读操作（循环进行很多次），
        //  一个线程进行修改操作（合适的时候执行一次）。读内存比读寄存器慢很多，循环一直去读的话，消耗就会很多，
        //  因此，频繁的读内存的值，就会非常低效，而且修改的线程迟迟不修改，读到的值一直是一样的值。
        //  因此，读的时候，就可能不从内存读数据了，而是直接从寄存器里面读。如果此时 把值修改了，那么就读不到这个值了。
        // 5、指令重排序：也是编译器优化的一种操作。就是调整代码的执行顺序，执行效果不变，但是效率就提高了。
        //  调整的前提也是逻辑不变。代码是单线程的话，一般不会出问题，如果是多线程的话，就可能出现问题
        //  避免问题还是通过 synchronized 来操作。
